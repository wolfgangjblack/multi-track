...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self, embedding_dim=128, device="auto"):
        super().__init__()
...
class VehicleTracker:
    ...
    def __init__(
        ...
        self.feature_extractor = FeatureExtractor(
            embedding_dim=feature_dim, device=device
        )
    ...
    def extract_features(
        ...
        batch = torch.from_numpy(np.stack(crops)).permute(0, 3, 1, 2).float() / 255.0
    ...
    def get_batch_detections(
        self, frames: List[np.ndarray], results
    ) -> List[List[Detection]]:
        """
            ...
            for box in boxes:
                ...
                if cls in self.yolo_labels:
                    ...
                    if len(batch_boxes) == self.batch_size:
                        ...
                        for i, ((conf, cls), bbox, feat) in enumerate(
                            zip(batch_info, batch_boxes, features)
                        ):
    ...
    def update(self, detections: List[Detection]) -> Dict[int, np.ndarray]:
        """
